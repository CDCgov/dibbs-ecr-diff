...
Note: This table only stores information for incoming eICRs/RRs, and only after they've been processed. During the augmentation step, Difference in Docs modifies the eICR and RR to use new deterministic setIds. These setIds are *not* stored.

## DynamoDB Table Design

For the purposes of Difference in Docs, we utilize a single DynamoDB table: `did-eicr-record`.

### Primary Key

DynamoDB supports either a single primary key (composing of just the Partition Key), or a composite primary key (composing of the Partition Key + the Sort Key). For this spec, we use the composite primary key as follows:
...
| `s3KeyDiffOutput` | string (S) | No | The S3 Object Key where the JSON diff output (if any) is stored. |
...
1. From the new message, get the S3 key of the eICR document and the S3 key of the RR Document. Fetch the document `D` using the S3 key, and parse it to retrieve the `setId`, `SET_ID` and the `versionNumber`, `VERSION_NUMBER`.
...
4. Execute the diff.
    * If a baseline `B` is found, fetch `B` from S3, run the diff, and determine if `D` has actionable changes.
       * At this point, the diff will be output to the S3 bucket
...
5. Once processed, the Lambda performs a `PutItem` to persist the new document to the `did-eicr-record` table:
    ...
    * Set `s3Key` to the S3 key received in message metadata.
    * Set `s3KeyRR` to the RR S3 key received in message metadata.
    * Set `s3KeyDiffOutput` to the S3 key of the diff output (if any)
...
1. An upstream system uploads the raw eICR to `s3://ecr-dev-data-repository/eicr_set_SET_ID_A_v3.xml`.
...
4. The Lambda downloads the file from `s3://ecr-dev-data-repository/eicr_set_SET_ID_A_v3.xml`.
    ...
    "Items": [
      {
        ...
        "s3Key": {"S": "s3://ecr-dev-data-repository/eicr_set_SET_ID_A_v1.xml"},
        "s3KeyRR": {"S": "s3://ecr-dev-data-repository/rr_set_SET_ID_A_v1.xml"},
        "s3KeyDiffOutput": {"NULL": true},
...
Note:
    ...
    * Since this is the earliest eICR in the bucket, it has no `comparedToVersion` or `s3KeyDiffOutput`.

7. The Lambda downloads the baseline file from the retrieved `s3Key` (`s3://ecr-dev-data-repository/eicr_set_SET_ID_A_v1.xml`).
...
10. The Lambda creates the diff output and its S3 key for storage (`s3://ecr-dev-data-repository/diff_v1_v3_0.json`).
...
11. Finally, the Lambda saves the record for version 3 in DynamoDB using `PutItem`:
  ...
  ```jsonc
  {
    ...
    "s3Key": {"S": "s3://ecr-dev-data-repository/eicr_set_SET_ID_A_v3.xml"},
    "s3KeyRR": {"S": "s3://ecr-dev-data-repository/rr_set_SET_ID_A_v3.xml"},
    "s3KeyDiffOutput": {"S": "s3://ecr-dev-data-repository/diff_set_SET_ID_A_v1-v3.json"},
...
### Out-of-Order Document Delivery
...
*Proposed MVP Behavior:* Process the late eICR version and diff it anyway
...
Cons: Compute potentially wasted on unused versions, S3 bucket storage space wasted on potentially unused diff outputs

#### Completely Ignore the late eICR version (Unimplemented solution)
...
Cons: Data that was added in version 2, but truncated between version 2 and 3 can potentially be lost
...
3. **DynamoDB Table Structure**. DynamoDB is schema-less, so we can write any objects we want to it, so long as they conform to the Partition and Sort Keys. However, should we have a "version number" for the shape of an Item in the DB? Can it be part of the Partition Key? Alternatively, could we store all data in a single Map (M) attribute?

4. **Storing the `documentId`.** APHL recently raised a concern that `documentID` may count as PII. Is it necessary to store the `documentId`? Instead of storing the `documentId`, we could create a hashed record ID that can't be traced back to the original document ID without a key that APHL controls. Is using this record ID preferable to relying on the primary key to identify unique documents?
