...
Note: This table only stores information for incoming eICRs/RRs, and only after they've been processed. The new deterministic setId that Difference in Docs generates for an eICR after it's been processed is not stored.
...
3. **S3 Storage Patterns:** Can we designate the S3 filenaming conventions with APHL? Will there be another S3 Bucket for Augmented eICRs generated by Difference in Docs?
...
5. **Storing the `documentId`.** APHL recently raised a concern that `documentID` may count as PII. Is it necessary to store the `documentId`? Instead of storing the `documentId`, we could create a hashed record ID that can't be traced back to the original document ID without a key that APHL controls. Is using this record ID preferable to relying on the primary key to identify unique documents?
