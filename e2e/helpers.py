...
def normalize_timestamps(content: str) -> str:
    """Normalizes timestamps in eICRs/RRs for consistent snapshot testing."""
    # grab the document-level time value generated by DID
    match = re.search(r'<effectiveTime value="([^"]+)"', content)

    if match:
        # replace that same augmentation time everywhere it appears in the XML
        # including <time> elements
        timestamp = match.group(1)
        content = content.replace(timestamp, "TIMESTAMP")

    # replace the generated timestamp in diff output, if present
    content = re.sub(
        r'"generatedAt": "[^"]+"',
...
class Uploader:
    ...
    def read_object_for_snapshot(self, key: str) -> str:
        return normalize_timestamps(self.read_object(key))

    def send_manifest(
        self, scenario: str, pairs: list[Pair]
    ...
    def _get_file_path(
        self, pair_id: int, type: Literal["eicr", "rr"], xml_paths: Iterable[Path]
    ) -> Path | None:
